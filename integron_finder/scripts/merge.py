...
def copy_file(out_dir, ext, *in_dirs):
    ...
    :param indirs: The path of the source directories
    :type indirs: list of str
    ...
    for _dir in in_dirs:
        ...
        for one_file in in_files:
            shutil.copy(one_file, os.path.join(out_dir, os.path.basename(one_file)))
...
def main(args=None, log_level=None):
    ...
    if os.path.exists(outdir):
        if not os.path.isdir(outdir):
            _log.critical("'{}' already exists and is not a directory".format(outdir))
            raise IOError()
    else:
