...
class TestGemBase(IntegronTest):
    ...
    #     file_names = (('ACBA.0917.00019', 'Draft', 2),
    #                   ('ESCO001.C.00001.C001', 'Complet', 1),
    #                   ('ACJO001.0321.00008.P008', 'Complet', 1))
    #     for file_name, gem_type, gem_vers in file_names:
    #         lst_path = self.find_data(os.path.join('Gembase', 'LSTINF', file_name + '.lst'))
    #         with self.subTest(lst_path=lst_path):
    #             vers_recieved, type_recieved = GembaseDB.gembase_sniffer(lst_path)
    #             self.assertEqual(type_recieved, gem_type)
    #             self.assertEqual(vers_recieved, gem_vers)
    #
    ...
    #     replicon_path = self.find_data(os.path.join('Gembase', 'Replicons', f'{replicon_name}.fna'))
    #     self.args.replicon = replicon_path
    #     cfg = Config(self.args)
    #     seq_db = read_multi_prot_fasta(replicon_path)
    #     replicon = next(seq_db)
    ...
    #     db = GembaseDB(replicon, cfg)
    #
    #     prots_info = db.gembase_draft_parser(lst_path, replicon_id)
    ...
    def test_get_description(self):
        ...
        for seq_name, ext in file_name:
            ...
            with self.catch_log():
                db = GembaseDB(replicon, cfg)
            print("\n###################################################")
            print(f"{db._gem_vers = }")
            print(f"{db._gem_type = }")
            print(db._info)
            print("###################################################")
