...
class TestFunctions(unittest.TestCase):
    ...
    def test_search_attc_uniq(self):
        ...
        attc_file = os.path.join("tests", "data", "Results_Integron_Finder_" + rep_name,
                                 "other", rep_name + "_attc_table.res")
        integron_finder.replicon_name = rep_name  # name of replicon in given fasta file
        ...
        attc_res = attc_res.append({"Accession_number": rep_name, "cm_attC": "attC_4",
    ...
    def test_search_attc_diff_strand(self):
        ...
        attc_df = attc_df.append({"Accession_number": rep_name, "cm_attC": "attC_4", "cm_debut": 1,
                                  "cm_fin": 47, "pos_beg": 15000, "pos_end": 16000, "sens": "+",
                                  "evalue": 0.19}, ignore_index=True)
        # search attC arrays, keeping palindromes
        # 2 attc sites are in the same array if they are on the same strand, and separated by
        # a distance less than 4kb
        integron_finder.DISTANCE_THRESHOLD = 4000  # (4kb at least between 2 different arrays)
        ...
        attc_res2 = pd.DataFrame(columns=["Accession_number", "cm_attC", "cm_debut", "cm_fin",
                                          "pos_beg", "pos_end", "sens", "evalue"])
        attc_res2 = attc_res2.append({"Accession_number": rep_name, "cm_attC": "attC_4",
